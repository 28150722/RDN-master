...
def load_images(paths, group=None, verbose=True, PLOT=False, AFLW=False, PLOT_shape=True):
    ...
    if AFLW:
      ...
      reference_shape = mio.import_pickle(Path('/home/hliu/data2/CongcongZhu/ICME/RDN/ckpt/pred') / 'reference_shape.pkl')
    else:
    ...
    centers = utils.k_means(shape_space, 300)
    centers = np.reshape(centers, [-1, 68, 2])
...
def load_images_aflw(paths, group=None, verbose=True, PLOT=True, AFLW=False, PLOT_shape=False):
    ...
    if AFLW:
      # reference_shape = PointCloud(mio.import_pickle(Path('/home/hliu/gmh/RL_FA/mdm_aflw/ckpt/train_aflw') / 'reference_shape.pkl'))
      reference_shape = mio.import_pickle(Path('/home/hliu/gmh/RL_FA/mdm_aflw/ckpt/train_aflw') / 'reference_shape.pkl')
    else:
      reference_shape = PointCloud(build_reference_shape(paths))
...
def load_images_test(paths, reference_shape, reference_3D, group=None, verbose=True, PLOT=False):
    """Loads and rescales input knn_2D to the diagonal of the reference shape.
