...
REGISTER_OP("PadPatches").Input("input: float32")
                             .Input("input2: int32")
                             .Input("input3: float32")
                             .Output("output: float32")
                            .Doc(R"doc(Pad patches from the input image. output: [k, num_patches, patch_height, patch_width, depth])doc");

class PadPatchesOp : public OpKernel {
 public:
  explicit PadPatchesOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const int32 k = offsets.shape().dim_size(0);
    OP_REQUIRES(context, offsets.shape().dims() == 3,
                errors::InvalidArgument("input must be a tensor [k, num_patches, 2]",
    ...
    TensorShape output_shape({k, num_patches, input_height, input_width, depth});
          ...
          for(int source_y=offset_y-patch_height/2, target_y=0;
            ...
            if (source_x > 0 && source_x < input_width && source_y > 0 && source_y < input_height) {
              for (int c = 0; c < depth; ++c) {
                output_patches(i, n, source_y, source_x, c) = input_images(0, source_y, source_x, c);
              }
            }
          }
        }
...
REGISTER_KERNEL_BUILDER(Name("PadPatches").Device(DEVICE_CPU),
                        PadPatchesOp);
