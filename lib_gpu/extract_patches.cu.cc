...
__global__ void ExtractPatchesGradKernel(typename TTypes<float, 4>::Tensor backprops,
			...
			for(int source_x=offset_x-patch_width/2, target_x=0;
              ...
              for(int source_y=offset_y-patch_height/2, target_y=0;
                ...
                if (source_x > 0 && source_x < input_width && source_y > 0 && source_y < input_height) {
                	for (int c = 0; c < depth; ++c) {
                		//output_patches(blockIdx.x, threadIdx.x, target_y, target_x, c) = input_images(blockIdx.x, source_y, source_x, c);
                		backprops(blockIdx.x, source_y, source_x, c) = gradients(blockIdx.x, threadIdx.x, target_y, target_x, c);
              		}
            	}
          	  }
        	}
...
void ExtractPatchesGradKernelLauncher(typename TTypes<float, 4>::Tensor backprops,
	...
	ThreadSize = pow(2,ThreadSize);
	ExtractPatchesGradKernel<<<BlockSize, ThreadSize>>>(backprops,
				                                       //input_images,
				                                       lms, 
				                                       gradients, 
				                                       batch_size, 
